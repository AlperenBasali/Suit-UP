<?php require_once('config.php') ?>

<?php include("partials/_header.php") ?>
<?php include("partials/body/_upbtn.php") ?>
<?php include("partials/body/navbar/_navbar.php") ?>
<?php include("partials/body/sec1/_sec1.php") ?>
<?php include("partials/body/sec2/_sec2.php") ?>
<?php include("partials/body/sec3/_sec3.php") ?>
<?php include("partials/body/sec4/_sec4.php") ?>
<?php include("partials/body/sec5/_sec5.php") ?>
<?php include("partials/body/sec6/_sec6.php") ?>
<?php include("partials/_footer.php") ?>


<!-- popular product kartlarını mysql den getirt. //?DONE -->
<!-- season sale kısmıda aynı şekilde //?DONE -->
<!-- bir ürüne bastığında gideceğin sayfayı oluştur o ürünün özel sayfası -->
<!-- ürünleri karta ekleme olayını yap -->
<!-- spesifik yönlendirmeleri yap (shoes,clothes...) -->